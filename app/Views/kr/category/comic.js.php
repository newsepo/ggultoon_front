<script>
    ...
    /* 전역 변수 기본값 세팅 */
    categoryParams.default(2, params);
            ...
            $.ajax({
                ...
                success: function (res) {
                    if (res.result) {
                        ...
                        // 검색 필터 바텀 시트 세팅
                        let sheetBody = `
                                            <input type="radio" name="comic_genre" id="genre_all" value="0" checked />
                        ...
                        if (res.data.list.length > 0) {
                            $.each(res.data.list, function (index, el) {
                                ...
                                // 검색 필터 바텀 시트 세팅
                                sheetBody += `
                                                <input type="radio" name="comic_genre" id="genre_`+ el.idx + `" value="` + el.idx + `"/>
    ...
    let importJs = {
        ...
        setting: function () {
            ...
            /*바텀 시트 장르 선택값 감지*/
            $("input:radio[name='comic_genre']").change(function () {

                // 장르 idx set
                let genreIdx = $("input[name='comic_genre']:checked").val();
