<script>
    ...
    /* 전역 변수 기본값 세팅 */
    categoryParams.default(1, params);
    ...
    let webtoon = {
        ...
        genreList: function () {
            ...
            $.ajax({
                ...
                beforeSend: function () {
                    let skeletonWrap = '<div class="skeleton-wrap" style="position:absolute; top:0; left: 0"></div>'
                ...
                success: function (res) {
                    if (res.result) {
                        ...
                        // 검색 필터 바텀 시트 세팅
                        let sheetBody = `
                                            <input type="radio" name="webtoon_genre" id="genre_all" value="0" checked />
                        ...
                        if (res.data.list.length > 0) {
                            $.each(res.data.list, function (index, el) {
                                ...
                                // 검색 필터 바텀 시트 세팅
                                sheetBody += `
                                                <input type="radio" name="webtoon_genre" id="genre_`+ el.idx + `" value="` + el.idx + `">
                                                <label for="genre_`+ el.idx + `">` + el.name + `</label>
                                            `;
                            });
                            $(".menuSwiper .swiper-wrapper").html(listBody);
                            $(".mySwiper .swiper-wrapper").append(slideBody);
    ...
    let importJs = {
        ...
        setting: function () {
            ...
            /*바텀 시트 장르 선택값 감지*/
            $("input:radio[name='webtoon_genre']").change(function () {

                // 장르 idx set
                let genreIdx = $("input[name='webtoon_genre']:checked").val();
